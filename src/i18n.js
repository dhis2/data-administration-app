export const i18nKeys = {
    dataIntegrity: {
        ...
        dataElementsWithoutDataSet: {
            ...
            ' entered into the system if a data element is not assigned to a data set. Choose ' +
            'Maintenance->Datasets->Edit from the main menu and then add the "orphaned" data element to the ' +
            'appropriate data set.',
        },
        dataElementsWithoutGroups: {
            label: 'Data elements without groups',
            text: 'Some Data Elements have been allocated to several Data Element Groups. This is currently not ' +
            'allowed, because it will result in duplication of linked data records in the PivotSource recordsets ' +
            'that `feed` the pivot tables. Go to Maintenance -> Data Element Groups to review each Data Element ' +
        ...
        dataElementsViolatingExclusiveGroupSets: {
            ...
            text: 'Some data elements have been allocated to several data element groups that are members of the ' +
            'same data element group set. All group sets in DHIS are defined as exclusive, which means that a data ' +
            'element can only be allocated to one data element group within that group set. ' +
            'Go to Maintenance -> Data elements and indicators ->Data element groups to review each data element ' +
            'identified in the integrity check. Either remove the data element from all groups except the one that ' +
            'it should be allocated to, or see if one of the groups should be placed in a different group set.',
        },
        dataElementsAssignedToDataSetsWithDifferentPeriodTypes: {
            ...
            ' The recommended approach would be to create two separate data elements (for instance a monthly and' +
        ...
        indicatorsWithoutGroups: {
            ...
            text: 'All Data Elements and Indicators must be assigned to at least one group, so these Indicators ' +
            'need to be allocated to their correct Data Element and Indicator Group. Go to Maintenance -> ' +
            'Indicator Groups, and allocate each of the `Orphaned` Indicators to its correct group.',
        },
        ...
        indicatorsViolatingExclusiveGroupSets: {
            ...
            ' the indicator from all groups except the one that it should be allocated to, or see if one of the' +
        ...
        organisationUnitsWithoutGroups: {
            ...
            text: 'All organisation units must be allocated to at least one group. The problem might either be that ' +
            ...
            'then you must first define them by going to Maintenance -> Organisation units->Organisation unit group ' +
            'sets and define at least one `compulsory` Group Set (the group set `OrgUnitType` are nearly universally ' +
            'relevant). If you have the relevant group sets, go to Maintenance -> OrgUnit Groups to review each ' +
        ...
        organisationUnitsViolatingExclusiveGroupSets: {
            ...
            text: 'Some organisation units have been allocated to several organisation unit groups that are ' +
            'members of the same organisation unit group set. All group sets in DHIS are defined as exclusive, ' +
            'which means that an organisation unit can only be allocated to one organisation unit group within that ' +
            'Group Set. For instance, one organisation unit cannot normally belong to the both the `Hospital` ' +
            'and `Clinic` groups, but rather to only to one of them. Go to Maintenance -> ' +
            'organisation unit->Organisation unit groups to review each organisation unit identified in ' +
            'the integrity check. Remove the organisation unit from all groups except the one that it should be ' +
            'allocated to.',
        },
        organisationUnitGroupsWithoutGroupSets: {
            ...
            ' Maintenance->Organisation unit->Organisation unit group sets and allocate the Organisation unit group' +
            ' to the appropriate group set.',
        },
        validationRulesWithoutGroups: {
            label: 'Validation rules without groups',
            text: 'All validation rules must be assigned to a group. Go to Services->Data quality->Validation' +
            ' rule group and assign the offending validation rule to a group.',
        },
        invalidValidationRuleLeftSideExpressions: {
            label: 'Invalid validation rule left side expressions',
            text: 'An error exists in the left-side validation rule definition. Go to ' +
            'Services->Data quality->Validation rule and click the "Edit" icon on the offending rule. Press ' +
            '"Edit left side" and make the corrections that are required.',
        },
        invalidValidationRuleRightSideExpressions: {
            label: 'Invalid validation rule right side expressions',
            text: 'An error exists in the right-side validation rule definition. Go to Services->' +
            'Data quality->Validation rule and click the "Edit" icon on the offending rule. Press "Edit right side" ' +
            'and make the corrections that are required.',
        },
